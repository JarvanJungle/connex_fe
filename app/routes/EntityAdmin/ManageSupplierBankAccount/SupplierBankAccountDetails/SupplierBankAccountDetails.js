...
import BANK_ACCOUNT_ROUTES_PATH from "../../ManageBankAccount/routes";
...
export default function SupplierBankAccountDetails() {
    ...
    const getDetailBankAccount = async (currentCompanyUUID, bankAccountUuid) => {
        ...
        if (responseCatalogue.data.status === "OK") {
            ...
            try {
                ...
                // if (resData.originalAccountUuid) {
                //     const resInternalConversationBankAccount = await ConversationService
                //         .getDetailInternalConversation(
                //             currentCompanyUUID, bankAccountUuid
                //         );
                //     if (resInternalConversationBankAccount.data.status === "OK") {
                //         resInternalConversationBankAccount?.data?.data?.conversations
                //             .forEach((item) => {
                //                 rowDataInternalConversation.push({
                //                     userName: item.sender,
                //                     userRole: item.designation,
    ...
    const handleCreateOrUpdate = async () => {
        try {
            ...
            if (pageState === PAGE_STAGE.CREATE) {
                ...
                // const supplier = bankAccountStates.listSupplier.find((item) => item.companyCode === value);
                // const dataToUpdate = {
    ...
    const handleChangeCountry = (event) => {
        ...
        const country = Countries.countries.find((item) => item.name === event.target.value);
        setBankAccountStates((prevStates) => ({
            ...prevStates,
                        ...
                        <Row className="mb-2">
                            <Col xs={12}>
                                ...
                                <Conversation
                                    ...
                                    defaultExpanded
                                    disabled={pageState !== PAGE_STAGE.CREATE && !isUpdate}
                                />
                            </Col>
                        </Row>
