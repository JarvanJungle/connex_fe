...
import { contentError, contentInfo, isNullOrUndefinedOrEmpty } from "helper/utilities";
import { useSelector } from "react-redux";
import CSVHelper from "helper/CSVHelper";
import _ from "lodash";
import { bankAccountColumnDefs } from "../helper";
...
export default function ListBankAccount() {
    ...
    const handleRolePermission = usePermission(FEATURE.BANK_ACCOUNT);
    ...
    const retrieveBankAccountList = async () => {
        ...
        if (!isNullOrUndefinedOrEmpty(currentCompanyUUID)) {
            const res = await BankAccountService.getAllBankAccount(currentCompanyUUID);
            if (res.data.status === "OK") {
                const listBankAccount = res.data.data;
                setListStates((prevStates) => ({
                    ...prevStates,
                    currentCompanyUUID,
                    listBankAccount
    ...
    const handleOnDrop = (data = []) => {
        ...
        try {
            for (let i = 0; i < data.length; i++) {
                if (data[i].data[0] !== "" && !data[i].data[0].includes("Bank Label")) {
                    const validationResult = CSVHelper.validateCSV(data, ["Bank Label", "Country", "Bank Name", "Bank Account No.", "Account Holder Name", "Currency"]);
                    ...
                    const uploadItem = {
                        bankLabel: data[i].data[0],
                        country: data[i].data[1],
                        bankName: data[i].data[2],
                        bankAccountNo: data[i].data[3],
                        accountHolderName: data[i].data[4],
                        currency: data[i].data[5],
                        swiftCode: data[i].data[6],
                        branch: data[i].data[7],
                        branchCode: data[i].data[8],
                        branchCity: data[i].data[9],
                        branchAddressLine1: data[i].data[10],
                        branchAddressLine2: data[i].data[11],
                        postalCode: data[i].data[12],
                        stateProvince: data[i].data[13],
                ...
                if (i !== 0 && data[i].data[0] === "") {
                    ...
                    throw new Error("Validate Error: Please select valid Bank Label");
                }
            }
        } catch (error) {
        ...
        if (massUpload.length !== 0) {
            BankAccountService.postMassUploadBankAccount(
    ...
    const handleExport = () => {
        listStates.gridApi
            .exportDataAsCsv({ fileName: CSVTemplates.Bank_Account_List_DownloadFileName });
    };

    const onRowDoubleClick = (event) => {
        history.push(`${BANK_ACCOUNT_ROUTES_PATH.BANK_ACCOUNT_DETAILS}?uuid=${event.data.uuid}`);
    };
    ...
    return (
        <Container fluid>
                            ...
                            {handleRolePermission?.write && (
                                <>
                                    ...
                                    <Button color="primary" className="mb-2 mr-2 px-3">
                                        <CSVLink
                                            data={CSVTemplates.Bank_Account_List_Data}
                                            headers={CSVTemplates.Bank_Account_List_Headers}
                                            filename={CSVTemplates.Bank_Account_List_TemplatesFilename}
                                            style={{ color: "white" }}
                                        >
                                            <i className="fa fa-download" />
                                            {" "}
                                    ...
                                    <Link to={BANK_ACCOUNT_ROUTES_PATH.BANK_ACCOUNT_CREATE}>
                                        <Button color="primary" className="mb-2 mr-2 px-3">
            ...
            <Row className="mb-3">
                <Col lg={12}>
                    <AgGridTable
                        columnDefs={bankAccountColumnDefs(t)}
